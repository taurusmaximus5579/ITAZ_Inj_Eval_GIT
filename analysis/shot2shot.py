...
def stats_with_percent(arr):
    ...
    if mean != 0:
        ...
        low_pct = 100 * (low - mean) / mean
        high_pct = 100 * (high - mean) / mean
    else:
...
def Eval_Shot2Shot(signal_dict, T, min_time_step, ICS_Eval_Result, hub_times, ordnerpfad,
    ...
    for sig_name in signal_names:
        ...
        if sig_name == "Needle Lift (mm)":
            ...
            all_stats["NeedleLift"] = {"StartTimes": st, "EndTimes": et, "MaxLift": ml, "MaxLiftTime": mlt}

            for x in [st["mean"], st["low"], st["high"], et["mean"], et["low"], et["high"]]:
                ax.axvline(x=x, color='red' if x in [st["mean"], et["mean"]] else 'orange', linestyle='--', linewidth=1.5)
            for x in [mlt["mean"], mlt["low"], mlt["high"]]:
                ax.axvline(x=x, color='green' if x == mlt["mean"] else 'lime', linestyle='--', linewidth=1.5)

            table_data = [
                ["", "Mean", "Std", "Std [%]", "-3σ", "-3σ [%]", "+3σ", "+3σ [%]"],
                ["Start Time [s]", f"{st['mean']:.6f}", f"{st['std']:.6f}", f"{st['std_pct']:.2f}", f"{st['low']:.6f}", f"{st['low_pct']:.2f}", f"{st['high']:.6f}", f"{st['high_pct']:.2f}"],
                ["End Time [s]", f"{et['mean']:.6f}", f"{et['std']:.6f}", f"{et['std_pct']:.2f}", f"{et['low']:.6f}", f"{et['low_pct']:.2f}", f"{et['high']:.6f}", f"{et['high_pct']:.2f}"],
                ["Max Lift [mm]", f"{ml['mean']:.6f}", f"{ml['std']:.6f}", f"{ml['std_pct']:.2f}", f"{ml['low']:.6f}", f"{ml['low_pct']:.2f}", f"{ml['high']:.6f}", f"{ml['high_pct']:.2f}"],
                ["Max Lift Time [s]", f"{mlt['mean']:.6f}", f"{mlt['std']:.6f}", f"{mlt['std_pct']:.2f}", f"{mlt['low']:.6f}", f"{mlt['low_pct']:.2f}", f"{mlt['high']:.6f}", f"{mlt['high_pct']:.2f}"],
            ]
            table = ax.table(cellText=table_data, cellLoc='center', loc='bottom', bbox=[0.0, -0.65, 1.0, 0.4])
        ...
        if sig_name == "Mass (mg)":
            ...
            all_stats["Mass"] = {"MaxMass": ms}

            table_data = [
                ["", "Mean [mg]", "Std", "Std [%]", "-3σ", "-3σ [%]", "+3σ", "+3σ [%]"],
                ["Max", f"{ms['mean']:.6f}", f"{ms['std']:.6f}", f"{ms['std_pct']:.2f}", f"{ms['low']:.6f}", f"{ms['low_pct']:.2f}", f"{ms['high']:.6f}", f"{ms['high_pct']:.2f}"],
            ]
            table = ax.table(cellText=table_data, cellLoc='center', loc='bottom', bbox=[0.0, -0.55, 1.0, 0.40])
            table.set_fontsize(10)
        ...
        if sig_name == "Injector Control Signal (A)":
            ...
            all_stats["ICS"] = {"ON_time": st_on, "OFF_time": st_off, "ON_duration": st_dur, "Boost": st_boost, "Hold": st_hold, "Zero": st_zero}

            for x in [st_on["mean"], st_on["low"], st_on["high"]]:
                ax.axvline(x=x, color='green' if x == st_on["mean"] else 'lime', linestyle='--', linewidth=1.5)
            for x in [st_off["mean"], st_off["low"], st_off["high"]]:
                ax.axvline(x=x, color='red' if x == st_off["mean"] else 'orange', linestyle='--', linewidth=1.5)
            ax.axvspan(st_on["mean"], st_off["mean"], color='yellow', alpha=0.15)

            table_data = [
                ["", "Mean", "Std", "Std [%]", "-3σ", "-3σ [%]", "+3σ", "+3σ [%]"],
                ["ICS ON time [s]", f"{st_on['mean']:.6f}", f"{st_on['std']:.6f}", f"{st_on['std_pct']:.2f}", f"{st_on['low']:.6f}", f"{st_on['low_pct']:.2f}", f"{st_on['high']:.6f}", f"{st_on['high_pct']:.2f}"],
                ["ICS OFF time [s]", f"{st_off['mean']:.6f}", f"{st_off['std']:.6f}", f"{st_off['std_pct']:.2f}", f"{st_off['low']:.6f}", f"{st_off['low_pct']:.2f}", f"{st_off['high']:.6f}", f"{st_off['high_pct']:.2f}"],
                ["ICS ON duration [s]", f"{st_dur['mean']:.6f}", f"{st_dur['std']:.6f}", f"{st_dur['std_pct']:.2f}", f"{st_dur['low']:.6f}", f"{st_dur['low_pct']:.2f}", f"{st_dur['high']:.6f}", f"{st_dur['high_pct']:.2f}"],
                ["Boost Median [A]", f"{st_boost['mean']:.6f}", f"{st_boost['std']:.6f}", f"{st_boost['std_pct']:.2f}", f"{st_boost['low']:.6f}", f"{st_boost['low_pct']:.2f}", f"{st_boost['high']:.6f}", f"{st_boost['high_pct']:.2f}"],
                ["Hold Median [A]", f"{st_hold['mean']:.6f}", f"{st_hold['std']:.6f}", f"{st_hold['std_pct']:.2f}", f"{st_hold['low']:.6f}", f"{st_hold['low_pct']:.2f}", f"{st_hold['high']:.6f}", f"{st_hold['high_pct']:.2f}"],
                ["Zero Median [A]", f"{st_zero['mean']:.6f}", f"{st_zero['std']:.6f}", f"{st_zero['std_pct']:.2f}", f"{st_zero['low']:.6f}", f"{st_zero['low_pct']:.2f}", f"{st_zero['high']:.6f}", f"{st_zero['high_pct']:.2f}"],
            ]
            table = ax.table(cellText=table_data, cellLoc='center', loc='bottom', bbox=[0.0, -0.65, 1.0, 0.45])
            table.set_fontsize(10)
        ...
        if sig_name == "Needle Lift Integrated (mm_s)":
            ...
            all_stats["NeedleLiftIntegrated"] = {"MaxIntegratedLift": st_int}

            table_data = [
                ["", "Mean [mm_s]", "Std", "Std [%]", "-3σ", "-3σ [%]", "+3σ", "+3σ [%]"],
                ["Max", f"{st_int['mean']:.6f}", f"{st_int['std']:.6f}", f"{st_int['std_pct']:.2f}", f"{st_int['low']:.6f}", f"{st_int['low_pct']:.2f}", f"{st_int['high']:.6f}", f"{st_int['high_pct']:.2f}"],
            ]
            table = ax.table(cellText=table_data, cellLoc='center', loc='bottom', bbox=[0.0, -0.55, 1.0, 0.40])
            table.set_fontsize(10)
            fig.subplots_adjust(bottom=0.32)
        ...
        if sig_name == "System Pressure (abs_bar)":
            ...
            all_stats["SystemPressure"] = {"p_start": st_start, "p_end": st_end, "p_min": st_minp, "t_min": st_mint, "delta_p": st_delta}

            table_data = [
                ["", "Mean", "Std", "Std [%]", "-3σ", "-3σ [%]", "+3σ", "+3σ [%]"],
                ["p(start) [bar]", f"{st_start['mean']:.6f}", f"{st_start['std']:.6f}", f"{st_start['std_pct']:.2f}", f"{st_start['low']:.6f}", f"{st_start['low_pct']:.2f}", f"{st_start['high']:.6f}", f"{st_start['high_pct']:.2f}"],
                ["p(end) [bar]", f"{st_end['mean']:.6f}", f"{st_end['std']:.6f}", f"{st_end['std_pct']:.2f}", f"{st_end['low']:.6f}", f"{st_end['low_pct']:.2f}", f"{st_end['high']:.6f}", f"{st_end['high_pct']:.2f}"],
                ["p(min) [bar]", f"{st_minp['mean']:.6f}", f"{st_minp['std']:.6f}", f"{st_minp['std_pct']:.2f}", f"{st_minp['low']:.6f}", f"{st_minp['low_pct']:.2f}", f"{st_minp['high']:.6f}", f"{st_minp['high_pct']:.2f}"],
                ["t(min) [s]", f"{st_mint['mean']:.6f}", f"{st_mint['std']:.6f}", f"{st_mint['std_pct']:.2f}", f"{st_mint['low']:.6f}", f"{st_mint['low_pct']:.2f}", f"{st_mint['high']:.6f}", f"{st_mint['high_pct']:.2f}"],
                ["Δp(start-min) [bar]", f"{st_delta['mean']:.6f}", f"{st_delta['std']:.6f}", f"{st_delta['std_pct']:.2f}", f"{st_delta['low']:.6f}", f"{st_delta['low_pct']:.2f}", f"{st_delta['high']:.6f}", f"{st_delta['high_pct']:.2f}"],
            ]
            table = ax.table(cellText=table_data, cellLoc='center', loc='bottom', bbox=[0.0, -1.30, 1.0, 0.70])
            table.set_fontsize(10)
