...
# -*- coding: utf-8 -*-
"""
18.01.2025 Lars Köhler
[✅] Hinzufügen des Integrierten Hubs (Needle Lift Integrated) mit zweiter y-Achse
"""
...
def GainCurve(signal_dict, T, step_size, ICS_Eval_Result, hub_times, ordnerpfad=None,
    ...
    # Versuche Needle Lift Dictionary zu laden
    ...
    for name, data in mass_dict.items():
        ...
        # Hole Needle Lift Daten für diesen Namen
        ...
        if end_index >= len(data):
            ...
            mass_info[name] = {
                ...
                "ICS_ON": ics_on_value,
                "NeedleLiftIntegrated": needle_value
            }

    # Arrays für Regression (ICS_ON vs Mass)
    x_vals, y_vals = [], []
    ...
    # HTML Plot mit Plotly und zweiter y-Achse
    fig_html = go.Figure()
    ...
    fig_html.update_layout(
        ...
        yaxis=dict(
            title=dict(text="Mass (mg)", font=dict(color='blue')),
            tickfont=dict(color='blue')
        ),
        yaxis2=dict(
            title=dict(text="Needle Lift Integrated (mm·s)", font=dict(color='green')),
    ...
    # Zusätzliche Ausgabe: Needle Lift Werte anzeigen
    print("\nZusätzliche Daten (Needle Lift Integrated):")
    for name, info in mass_info.items():
        print(f"{name}: Mass={info['mass']}, ICS_ON={info['ICS_ON']}, NeedleLiftIntegrated={info['NeedleLiftIntegrated']}")
