...
def analyze_and_plot_needle_lifts(signal_dict, T, ordnerpfad=None):
    ...
    for name, data in needle_lifts.items():
        ...
        # Integration über das gesamte Signal (für Vergleich)
        integrated_lift = cumulative_trapezoid(data, dx=dt, initial=0)
        
        # Integration nur im Hubfenster, aber gleiche Länge wie T
        integrated_window = np.zeros_like(integrated_lift)
        
        if start_idx is not None and end_idx is not None:
            # Teilintegration im Fenster
            partial = cumulative_trapezoid(
                data[start_idx:end_idx+1],
                dx=dt[start_idx:end_idx],
                initial=0
            )
            ...
            # Nach dem Ende → letzten Wert halten
