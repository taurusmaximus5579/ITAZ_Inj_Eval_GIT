...
@author: larsk
[✅] Bilder erstellen inkl. Einzelplots direkt unter 'Bilder'
"""
...
def analyze_plateaus(signal_dict, boost_range, hold_range, zero_range,
    ...
    # Hauptdiagramm: Alle Stromsignale
    ...
    for idx, (key, strom_raw) in enumerate(strom_signale.items()):
        ...
        if np.any(mask_above):
            ...
            plt.axvline(x=T_first_shifted, color="blue", linestyle="--", linewidth=1)
            plt.axvline(x=T_last_shifted, color="red", linestyle="--", linewidth=1)

        result_all[key] = result
        plt.plot(T, strom, label=f"{key}", color=colors[idx % len(colors)])

        for name, median in medians.items():
            plt.axhline(y=median, color=plateau_colors[name], linestyle="--", linewidth=1)

        # Einzelplot für jedes Stromsignal direkt unter Bilder speichern
        plt.figure(figsize=(10, 5))
        plt.plot(T, strom, color=colors[idx % len(colors)], label=f"{key}")
        plt.title(f"Stromsignal: {key}")
        plt.xlabel("Time")
        plt.ylabel("Injector Control Signal (A)")
        plt.grid(True)

        # Plateau-Linien einzeichnen
        for name, median in medians.items():
            plt.axhline(y=median, color=plateau_colors[name], linestyle="--", linewidth=1, label=f"{name}-Median")

        # ICS_ON / ICS_OFF Linien einzeichnen
        if "ICS_ON_time" in result and "ICS_OFF_time" in result:
            plt.axvline(x=result["ICS_ON_time"], color="blue", linestyle="--", linewidth=1, label="ICS_ON")
            plt.axvline(x=result["ICS_OFF_time"], color="red", linestyle="--", linewidth=1, label="ICS_OFF")

        plt.legend()
        plt.tight_layout()
        einzel_plot_path = os.path.join(bilder_pfad, f"{key}_Stromsignal.png")  # Direkt unter Bilder
        plt.savefig(einzel_plot_path)
        print(f"✅ Einzelplot gespeichert: {einzel_plot_path}")
        plt.close()
    ...
    print(f"✅ Hauptdiagramm gespeichert: {main_plot_path}")
    plt.close()
