...
@author: larsk
[✅] Ströme und Zeiten in Diagramm eintragen
"""
...
def analyze_plateaus(signal_dict, boost_range, hold_range, zero_range,
    ...
    for idx, (key, strom_raw) in enumerate(strom_signale.items()):
        ...
        # Hauptdiagramm: Stromsignal zeichnen
        ...
        for name, median in medians.items():
            plt.axhline(
                y=median,
                color=plateau_colors[name],
                linestyle="--",
                linewidth=1,
                label=f"{name}-Median = {median:.3f}A"
            )

        # ICS Linien
        if "ICS_ON_time" in result and "ICS_OFF_time" in result:
            plt.axvline(result["ICS_ON_time"], color="blue", linestyle="--", linewidth=1,
                        label=f"ICS On = {result['ICS_ON_time']:.6f}s")
            plt.axvline(result["ICS_OFF_time"], color="red", linestyle="--", linewidth=1,
                        label=f"ICS Off = {result['ICS_OFF_time']:.6f}s")
            plt.axvline(result["ICS_OFF_time"], color="purple", linestyle=":", linewidth=1,
                        label=f"Duration = {result['ICS_ON']:.6f}s")
    ...
    # ---------------------------------------------------------
    # BOOST / HOLD / ZERO / ICS ON / ICS OFF – ALLE EINZELNEN LINIEN
    # ---------------------------------------------------------
